# QFun Plugin API 参考文档

> 脚本引擎基于 BeanShell，支持 Java 8 语法，不支持注解。
...
- **线程模型**：事件回调默认在 IO 线程执行。操作 UI 需切换至主线程，内置 `toast` 已自动处理线程切换。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | `int` | 聊天类型（1:好友/私聊, 2:群聊, 100:陌生人） |
| `msgType` | `int` | 消息类型 |
| `peerUin` | `String` | 群号或好友QQ号 |
| `peerUid` | `String` | 群号或好友UID |
| `userUin` | `String` | 发送者QQ号 |
| `userUid` | `String` | 发送者UID |
| `time` | `long` | 发送时间戳（秒） |
| `msgId` | `long` | 消息ID |
| `msg` | `String` | 文本消息内容（包含 `[pic=url]` 等格式） |
| `path` | `String` | 文件/视频/语音保存路径 |
| `atList` | `List<String>` | 消息中艾特的QQ号列表 |
| `atMap` | `Map<String, String>` | 艾特映射表（Key: Uin, Value: 艾特内容） |
| `data` | `Object` | 原始 `MsgRecord` 对象 |
| `contact` | `Object` | 原始 `Contact` 对象 |
...
| `groupOwner` | `String` | 群主QQ号 |
| `groupInfo` | `Object` | 原始 `TroopInfo` 对象 |
...
|------|------|------|
| `uin` | `String` | 成员QQ号 |
| `uinName` | `String` | 群名片/昵称 |
| `uinLevel` | `int` | 群等级 |
| `joinGroupTime` | `long` | 入群时间戳 |
| `lastActiveTime` | `long` | 最后发言时间戳 |
| `role` | `String` | 角色（OWNER:群主, ADMIN:管理员, MEMBER:成员） |
| `memberInfo` | `Object` | 原始 `TroopMemberInfo` 对象 |

### 5. ForbidInfo（禁言信息）

| 字段 | 类型 | 说明 |
|------|------|------|
| `user` | `String` | 被禁言成员QQ号 |
...
- `sendMsg(String peerUin, String 内容, int 聊天类型)`
  - 聊天类型：1好友 / 2群聊 / 100陌生人
  - 支持格式：
    - 艾特：`[atUin=QQ号]`（QQ号为0时表示艾特全体）
    - 图片：`[pic=图片链接或绝对路径]`
    - 可任意组合，将根据形式自动解析
- `sendMsg(Object contact, String 内容)`

#### 2. sendPic：发送图片

- `sendPic(String peerUin, String 图片路径, int 聊天类型)`
- `sendPic(Object contact, String 图片路径)`

#### 3. sendPtt：发送语音

- `sendPtt(String peerUin, String 语音路径, int 聊天类型)`
- `sendPtt(Object contact, String 语音路径)`

#### 4. sendCard：发送 JSON 卡片

- `sendCard(String peerUin, String JSON字符串, int 聊天类型)`
- `sendCard(Object contact, String JSON字符串)`

#### 5. sendFile：发送文件

- `sendFile(String peerUin, String 文件路径, int 聊天类型)`
- `sendFile(Object contact, String 文件路径)`

#### 6. sendVideo：发送视频

- `sendVideo(String peerUin, String 视频路径, int 聊天类型)`
- `sendVideo(Object contact, String 视频路径)`

#### 7. sendReplyMsg：发送引用回复

- `sendReplyMsg(String peerUin, long 引用消息ID, String 内容, int 聊天类型)`
- `sendReplyMsg(Object contact, long 引用消息ID, String 内容)`

#### 8. recallMsg：撤回消息

- `recallMsg(int 聊天类型, String peerUin, long 消息ID)`
- `recallMsg(Object contact, long 消息ID)`
...
`void onMsg(Object msgData)` — 参数为 `MsgData` 对象。
...
`String getMsg(String 原始内容)` — 发送文本消息前触发，返回修改后的文本内容。
...
```java
log(String 文件名, String 内容)           // 追加写入日志到脚本目录（默认文件名 "log.txt"）
...
registerActivity(Class<? extends Activity> 类)  // 注册后可使用startActivity启动
```

jar、dex加载后可直接使用 import 导入。
